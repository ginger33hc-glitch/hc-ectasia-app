...
def runtime_invariants():
    ...
    # Approved CER-AI boundary: CCT <=480 µm is a hard stop and therefore not scoreable.
    if [core.lasik_pachy_points(x) for x in (479, 480, 481, 499, 500, 509, 510, 511)] != [None, None, 2, 2, 1, 1, 0, 0]:
        errors.append("CER-AI pachymetry policy is not active: expected <=480 hard stop, 481-499 +2, 500-509 +1, >=510 +0")
    ...
    try:
        erss=composition.erss_topography_guard;erss_evidence=composition.erss_topography_evidence_policy;targeted=composition.pentacam_targeted_reread
        ...
        if core.assess_eye is not erss_evidence.assess_eye_with_i_s_evidence:errors.append("ERSS I-S evidence gate is not the active eye-assessment layer")
        if core.required_tomography_missing is not erss_evidence.required_tomography_missing_with_i_s:errors.append("ERSS I-S evidence gate is not the active tomography requirement layer")
    ...
    checks=(("_erss_visual_morphology_policy_installed","Improved ERSS visual morphology policy is not active"),("_randleman_bad_independence_installed","BAD-independent Randleman ERSS pathway is not active"),("_hc_final_decision_hierarchy_installed","CER-AI final BAD-D/Randleman decision hierarchy is not active"),("_hc_status_rank_policy_installed","CER-AI aggregate status ranking is not active"),("_hc_inter_eye_tomography_policy_installed","Automated inter-eye tomography concern layer is not active"),("_hc_microkeratome_planning_installed","Post-assessment ML7 microkeratome planning layer is not active"),("_hc_nice_installed","Independent CER-AI NICE policy is not active"),("_hc_readiness_installed","Pre-report readiness workflow is not active"),("_cerai_report_builders_installed","Explicit report-builder installation is not active"),("_cerai_named_user_access_installed","Named-user access boundary is not active"),("_cerai_operational_security_installed","Operational security boundary is not active"),("_cerai_case_archive_installed","Encrypted case archive boundary is not active"),("_cerai_audit_log_installed","Encrypted audit-log boundary is not active"),("_cerai_case_catalog_installed","Encrypted case catalog boundary is not active"),("_cerai_historical_report_installed","Historical report regeneration boundary is not active"),("_cerai_research_export_installed","Research export boundary is not active"),("_cerai_named_user_ui_installed","Named-user archive UI boundary is not active"),("_erss_topography_evidence_policy_installed","ERSS I-S/topography evidence gate is not active"),("_erss_auto_read_policy_installed","ERSS morphology auto-read separation policy is not active"),("_cerai_targeted_pentacam_reread_installed","Targeted Pentacam numeric reread layer is not active"),("_hc_lasik_fallback_installed","LASIK fallback planner is not active"))
    for attr,msg in checks:
        if not getattr(core,attr,False):errors.append(msg)
    if composition.microkeratome_planning_policy._previous_hc_engine is not composition.inter_eye_tomography_policy.hc_engine_with_inter_eye_tomography:errors.append("Inter-eye to ML7 runtime wrapper order is invalid")
    if core.hc_engine is not composition.erss_auto_read_policy.hc_engine_with_erss_auto_read:errors.append("ERSS auto-read cleanup is not the outermost clinical engine layer")
    ...
    if not getattr(app.state,"cerai_canonical_runtime_ready",False):errors.append("Canonical ASGI startup marker is not active")
