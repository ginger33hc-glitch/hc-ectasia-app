...
Transcription only. This module does not score PS3 and does not calculate SRAX.
Approved sources:
- topographic_astig_D: Show 2 Exams > Topometric > Cornea Front > Astig.
- topographic_steep_axis_deg: same section > Axis (steep).
- posterior_Kmean_D: Show 2 Exams > Topometric > Cornea Back > Km.
- F_Ele_Th_um: BAD/Belin-Ambrosio Display > labeled F. Ele.Th box.
- B_Ele_Th_um remains owned by the existing NICE BAD-display B. Ele.Th pathway.
No map spot, generic elevation field, alternate screen, or calculated substitute is accepted.
"""
...
def _is_bad_display(result,eye):
    return any(t in {"BAD_DISPLAY","BELIN_AMBROSIO_DISPLAY","BELIN_AMBROSIO_ENHANCED_ECTASIA_DISPLAY"} or ("BELIN" in t and "AMBROSIO" in t) for t in _screen_tokens(result,eye))
...
def _provenance_source(eye,field):
    items=(eye.get("field_provenance") or {}).get(field) or []
    return {_normalized_token(i.get("source")) for i in items if isinstance(i,dict)}
def _source_ok(result,eye,field):
    sources=_provenance_source(eye,field)
    if field=="F_Ele_Th_um":
        return _is_bad_display(result,eye) and (not sources or "BAD_DISPLAY_F_ELE_TH_LABELED_BOX" in sources)
    if not _is_show2_topometric(result,eye):return False
    allowed={
        "topographic_astig_D":{"SHOW_2_EXAMS_TOPOMETRIC_CORNEA_FRONT_ASTIG","PENTACAM_SHOW_2_EXAMS_TOPOMETRIC_CORNEA_FRONT_ASTIG"},
        "topographic_steep_axis_deg":{"SHOW_2_EXAMS_TOPOMETRIC_CORNEA_FRONT_AXIS_STEEP","PENTACAM_SHOW_2_EXAMS_TOPOMETRIC_CORNEA_FRONT_AXIS_STEEP"},
        "posterior_Kmean_D":{"SHOW_2_EXAMS_TOPOMETRIC_CORNEA_BACK_KM","PENTACAM_SHOW_2_EXAMS_TOPOMETRIC_CORNEA_BACK_KM"},
    }[field]
...
def _bad_b_ele_th_candidates(results,eye_name):
    ...
    for result in results:
        for reading in result.get("nice_readings") or []:
            if reading.get("eye")!=eye_name:continue
            if reading.get("b_ele_th_status")=="CONFIDENT" and reading.get("b_ele_th_landmark")=="B_ELE_TH_LABELED_BOX" and reading.get("b_ele_th_page")=="BAD_DISPLAY" and _number(reading.get("B_Ele_Th_um")):values.append(float(reading["B_Ele_Th_um"]))
    return values

def merge_extractions_with_ps3_fields(results):
    if _previous_merge_extractions is None:raise RuntimeError("PS3 extraction adapter not initialized")
    merged=_previous_merge_extractions(results);by_eye={x.get("eye"):x for x in merged.get("eyes",[]) if x.get("eye") in {"OD","OS"}}
        ...
        for field in PS3_EXTRA_FIELDS:
            ...
            for result in results:
                for source_eye in result.get("eyes") or []:
                    ...
                    value=source_eye.get(field);is_verified=field in set(source_eye.get("table_verified_numeric_fields") or [])
                    if is_verified and _number(value):candidates.append(float(value))
            unique=[]
...
def install(core):
    ...
    if hasattr(core,"merge_extractions"):
        _previous_merge_extractions=core.merge_extractions;core.merge_extractions=merge_extractions_with_ps3_fields
