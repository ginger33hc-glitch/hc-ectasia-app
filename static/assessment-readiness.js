...
window.HCReadiness = class {
    ...
    for(const input of this.panel.querySelectorAll('[data-measurement]')) {
      ...
      if(input.tagName!=='SELECT') {
        ...
        if(!/^[+-]?(?:\d+(?:\.\d*)?|\.\d+)$/.test(raw)||!Number.isFinite(Number(raw)))
          throw new Error(`Enter a valid number for ${input.dataset.eye} ${input.dataset.measurement}.`);
        value=Number(raw);
      }
  ...
  show(response) {
    ...
    const heading=document.createElement('h3'); heading.textContent='Required information — no report has been generated';
    this.panel.append(heading);
    const note=document.createElement('p');note.textContent='Complete all items below, then continue. Existing inputs and image readings are retained. No calculation is required from the surgeon.';this.panel.append(note);
    const seen=new Set();
    for(const item of response.input_requests||[]) {
      const identity=[item.eye,item.form_id||item.key,item.kind].join(':');
      if(seen.has(identity))continue;seen.add(identity);
      const row=document.createElement('div');row.className='row';
      const label=document.createElement('label');label.textContent=`${item.eye}: ${item.label}`;row.append(label);
      let input;
      if(item.kind==='form') {
        ...
        if(item.kind==='select')for(const value of ['',...(item.options||[])]){const option=document.createElement('option');option.value=value;option.textContent=value||'Select';input.append(option);}
      ...
      else {const help=document.createElement('span');help.textContent=item.help;row.append(help);}
