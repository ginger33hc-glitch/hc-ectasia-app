(() => {
  ...
  const html = `<section id="hcReferenceAppendix" class="hc-reference-appendix"><h3>CERAI BAD-D reference points</h3><table class="clinical-table"><thead><tr><th>Final BAD-D</th><th>CERAI interpretation / action</th></tr></thead><tbody><tr><td>≤ 1.6</td><td class="hc-bad-normal">NORMAL</td></tr><tr><td>&gt; 1.6 to &lt; 3.0</td><td class="hc-bad-suspicious">SUSPICIOUS — contextual finding; final decision follows the CERAI hierarchy</td></tr><tr><td>≥ 3.0</td><td class="hc-bad-abnormal">ABNORMAL CORNEA — DO NOT PROCEED</td></tr></tbody></table><h3>Published Randleman / ERSS scoring points</h3><table class="clinical-table"><thead><tr><th>Variable</th><th>Finding</th><th>Points</th></tr></thead><tbody>${body}</tbody></table><p class="note"><strong>Independent pathways:</strong> Randleman anterior-topography points come only from the anterior curvature/topography image; BAD/BAD-D is not used. On Pentacam 4 Maps Refractive the source is the upper-left Axial/Sagittal Curvature (Front) panel. Published ERSS total: 0–2 low, 3 moderate, ≥4 high.</p><p class="note"><strong>Active CERAI pachymetry policy:</strong> &lt;480 µm = hard stop; 480–499 µm = +2; 500–509 µm = +1; ≥510 µm = +0.</p><p class="note"><strong>CERAI modification:</strong> the active CERAI engine intentionally uses CERAI-modified age and pachymetry rules. The patient score must therefore be read from the CERAI score breakdown, not reconstructed from this published reference table.</p><p class="note"><strong>Inter-eye tomography:</strong> assessed automatically from bilateral categorical tomography findings. It is a non-scored contextual concern and does not independently change the CERAI disposition.</p></section>`;
  let queued = false;

  function installStaticPolicyUI() {
    document.title = "CERAI v0.7.50";
    const heading = document.querySelector("body > header h1");
    if (heading) heading.textContent = "CERAI v0.7.50";
