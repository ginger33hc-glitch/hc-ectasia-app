...
def assess_eye_with_bad_independent_randleman(eye, plan, age, patient_modifiers):
    ...
    # Publish ERSS whenever its own five inputs exist, even if the separate CER-AI
    ...
    warnings.append(
        "RANDLEMAN/BAD SEPARATION: Randleman ERSS is calculated only from anterior topography, RSB, age, preoperative corneal thickness, and manifest MRSE. BAD-D/Belin-Ambrosio data are a separate CER-AI tomography pathway and are never required to calculate the Randleman score."
    )
