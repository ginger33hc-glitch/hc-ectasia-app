...
def test_clean_lasik_score_components_match_canonical_primitives_across_matrix():
    ...
    for age, pachy, morphology, expected_rsb, mrse in cases:
        # Choose flap/ablation values that produce the desired RSB directly.
        flap = 100
...
def test_clean_surgical_outputs_match_canonical_formulas_for_comparable_cases():
    lasik = clean_case(pachy_thinnest_um=520, flap_um=100, ablation_um=60, preop_kmean_d=44, intended_mrse_d=-3)
    assert lasik.calculations.lasik_rsb_um == legacy.lasik_rsb(520, 100, 60)
    assert lasik.calculations.lasik_pta_percent == legacy.lasik_pta(520, 100, 60)
    assert lasik.calculations.final_kmean_d == legacy.final_kmean(44, -3)

    prk = clean_case(procedure="PRK", flap_um=None, pachy_thinnest_um=520, ablation_um=60)
    assert prk.calculations.prk_rst_um == legacy.prk_rst(520, 60)
    assert prk.calculations.prk_pta_percent == legacy.prk_pta(520, 60)


def test_clean_final_lasik_status_matches_locked_principal_hierarchy_for_comparable_cases():
    # These cases avoid intentional clean-vs-legacy architecture corrections.
    expected = (
        ({}, "PASS WITH CAUTION"),
