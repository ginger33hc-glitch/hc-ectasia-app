...
This complements primitive equivalence tests. Intentional CER-AI source-readiness
requirements are excluded from status equivalence and remain locked by dedicated tests.
"""
...
def clean_case(**changes):
    values = dict(age_years=30, pachy_thinnest_um=520, bad_d=1.0,
        morphology="NORMAL_SYMMETRIC", procedure="LASIK", prior_refractive_surgery=False,
        ablation_um=60, flap_um=100, preop_kmean_d=44, manifest_mrse_d=-3,
        intended_mrse_d=-3, intended_sphere_d=-3, intended_cylinder_magnitude_d=0,
...
def test_clean_bad_d_classification_matches_canonical_across_final_boundaries():
    for bad_d in (1.0,1.6,1.6001,2.5999,2.6,3.0,4.0):
        assert clean_case(bad_d=bad_d).bad_d_status==legacy.bad_classification(bad_d,final=True)
...
def test_shared_lasik_calculations_match_when_canonical_source_readiness_is_stricter():
