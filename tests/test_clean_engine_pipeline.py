...
def test_fallback_plan_a_success_stops_after_a():
    out = assess(base(ablation_um=60))
...
def test_fallback_plan_a_rsb_failure_is_rescued_by_plan_b_recalculation():
    out = assess(base(pachy_thinnest_um=500, ablation_um=101, intended_sphere_d=-3, intended_cylinder_magnitude_d=1, intended_mrse_d=-3.5))
...
def test_fallback_hyperopic_plan_b_cannot_invent_ablation():
    out = assess(base(pachy_thinnest_um=500, ablation_um=101, intended_sphere_d=2, intended_mrse_d=1.5, intended_cylinder_magnitude_d=1))
    assert [x.plan_name for x in out.lasik_planning_sequence] == ["Plan A", "Plan B"]
...
def test_pipeline_exposes_and_sums_all_five_lasik_erss_components():
    out = assess(base(age_years=19, pachy_thinnest_um=505, morphology="ASYMMETRIC_BOWTIE", flap_um=100, ablation_um=145, manifest_mrse_d=-9))
    assert (out.scores.age_points, out.scores.pachymetry_points, out.scores.topography_points, out.scores.rsb_points, out.scores.mrse_points, out.scores.erss_total) == (2, 1, 1, 0, 1, 5)
...
def test_pachymetry_480_scores_and_479_is_independent_hard_stop():
    scoring = assess(base(pachy_thinnest_um=480))
...
def test_lasik_rsb_just_below_300_triggers_automatic_fallback():
    rescued = assess(base(pachy_thinnest_um=490, flap_um=100, ablation_um=90.001))
    boundary = assess(base(pachy_thinnest_um=490, flap_um=100, ablation_um=90))
    assert [step.plan_name for step in rescued.lasik_planning_sequence] == ["Plan A", "Plan B"]
    assert rescued.lasik_planning_sequence[0].rsb_um < 300
    assert "LASIK_RSB_LT_300" not in rescued.hard_stops
    assert [step.plan_name for step in boundary.lasik_planning_sequence] == ["Plan A"]
...
def test_final_kmean_boundaries_are_inclusive():
    assert "FINAL_KMEAN_OUTSIDE_36_48" not in assess(base(preop_kmean_d=40, intended_mrse_d=-5, intended_sphere_d=-5)).hard_stops
    assert "FINAL_KMEAN_OUTSIDE_36_48" not in assess(base(preop_kmean_d=44, intended_mrse_d=5, intended_sphere_d=5)).hard_stops
    assert "FINAL_KMEAN_OUTSIDE_36_48" in assess(base(preop_kmean_d=44, intended_mrse_d=5.001, intended_sphere_d=5.001)).hard_stops


def test_myopic_refractive_magnitude_boundary_is_strict():
    assert "INTENDED_SPHERE_LT_MINUS_10" not in assess(base(intended_sphere_d=-10.0, intended_mrse_d=-10.0)).hard_stops
    assert "INTENDED_SPHERE_LT_MINUS_10" in assess(base(intended_sphere_d=-10.001, intended_mrse_d=-10.001)).hard_stops


def test_hyperopic_refractive_magnitude_boundary_is_strict():
    assert "INTENDED_SPHERE_GT_PLUS_6" not in assess(base(intended_sphere_d=6.0, intended_mrse_d=6.0)).hard_stops
    assert "INTENDED_SPHERE_GT_PLUS_6" in assess(base(intended_sphere_d=6.001, intended_mrse_d=6.001)).hard_stops


def test_all_procedure_critical_inputs_fail_closed():
    for field in ("preop_kmean_d", "intended_mrse_d", "intended_sphere_d", "intended_cylinder_magnitude_d", "laser_platform"):
        out = assess(base(**{field: None}))
        assert out.status == "DATA INSUFFICIENT"
        assert field in out.missing
    missing_ablation = assess(base(
        ablation_um=None, intended_sphere_d=2,
        intended_cylinder_magnitude_d=0, intended_mrse_d=2,
    ))
    assert missing_ablation.status == "DATA INSUFFICIENT"
    assert "ablation_um" in missing_ablation.missing
    for field in ("manifest_mrse_d",):
        out = assess(base(**{field: None}))
        assert out.status == "DATA INSUFFICIENT"
        assert field in out.missing
...
def test_prior_refractive_surgery_short_circuits_virgin_engine():
    out = assess(base(prior_refractive_surgery=True, pachy_thinnest_um=430, bad_d=3.2))
    assert out.status == "POST-REFRACTIVE PATHWAY REQUIRED"
    assert not out.hard_stops
    assert out.scores.erss_total is None


def test_mixed_plan_does_not_use_mrse_kmean_shortcut():
    out = assess(base(
        preop_kmean_d=47.5, intended_sphere_d=2,
        intended_cylinder_magnitude_d=4, intended_mrse_d=0,
    ))
    assert out.calculations.final_kmean_d is None
    assert "FINAL_KMEAN_OUTSIDE_36_48" not in out.hard_stops
