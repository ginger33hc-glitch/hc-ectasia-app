...
all active tests while explicitly removing only the 13 expectations that were
retired before the frozen launch contract.
"""
...
if _SPEC is None or _SPEC.loader is None:
    raise RuntimeError("Unable to load historical HC-engine regression module")
...
for _class_name, _method_names in _RETIRED_METHODS.items():
    ...
    for _method_name in _method_names:
        if not hasattr(_class, _method_name):
            raise RuntimeError(f"Retired HC-engine test method missing: {_class_name}.{_method_name}")
        delattr(_class, _method_name)

# Re-export the historical module's public surface so current fixtures/importers
# continue to see the same helpers and TestCase classes, minus retired methods.
for _name, _value in vars(_legacy).items():
    if not _name.startswith("__"):
        globals()[_name] = _value

# Keep the retirement manifest available for architecture tests without exposing
# helper loop variables as accidental test globals.
PHASE4_RETIRED_HC_ENGINE_METHODS = {
    key: tuple(sorted(value)) for key, value in _RETIRED_METHODS.items()
}
