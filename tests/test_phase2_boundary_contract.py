"""Boundary locks for canonical pre-core readiness and clinical-core separation."""
...
def test_missing_source_still_blocks_before_scoring():
    items = _mandatory_set(); items.pop()
...
def test_precore_readiness_consumes_single_contact_lens_policy(modifiers):
    expected = contact_lens_washout(modifiers)
    actual = evaluate_precore_readiness(
        age_years=30,
        eye_plans={"OD": {"prior": "no", "procedure": "LASIK"}},
        patient_modifiers=modifiers,
    )
    assert actual["contact_lens_washout"] == expected
...
def test_clinical_core_does_not_own_transport_report_or_archive_layers():
    ...
    forbidden_imports = (
        "import reports", "from reports", "import case_archive", "from case_archive",
        "import assessment_workflow", "from assessment_workflow",
        "import mandatory_source_set_policy", "from mandatory_source_set_policy",
...
def test_pipeline_documents_external_boundaries_explicitly():
    ...
    for phrase in (
        "readiness", "identity/source validation", "contact-lens washout",
        "clinical eligibility", "planning fallback", "reporting", "archive",
    ):
