...
| LASIK pachymetry bands | Apply explicit HC-modified bands rather than silently inheriting ambiguous published table edges | Not addressed | `<480 µm` hard stop; `480–499 µm` = +2; `500–509 µm` = +1; `>=510 µm` = +0 |
| ML7 post-assessment planning | Keep device planning separate from ectasia eligibility; use the active Turkish ML7 reference and HC hinge amendment only after a favorable LASIK result | Not integrated | Status-independent module reports ring/pressure/blade and hinge recommendations without changing the ectasia disposition. For K spread `>4.00 D`, perpendicular-to-steep-axis hinge is primary; the `+10` temporal/nasal contingency requires projected RSB `>=300 µm` and PTA `<40%` and remains anatomy-dependent |
...
| Pachymetry policy | HC hard stop `<480 µm`; exactly 480 enters scoring | Implemented | Preserved and boundary-tested; all hard stops are accumulated even when other data are missing |
