# CERAI v0.7 — Source Compliance Audit

Audit date: 26 August 2026
Protocol: CERAI Preoperative Ectasia Risk Assessment for Corneal Refractive Surgery
Evidence set: the 10 supplied source/review files dated through 24 August 2026, focused hyperopic/mixed literature and WaveLight labeling reviewed 27 August 2026, plus binding CERAI operational amendments.
...
| LASIK pachymetry bands | Apply explicit CERAI-modified bands rather than silently inheriting ambiguous published table edges | Not addressed | `<480 µm` hard stop; `480–499 µm` = +2; `500–509 µm` = +1; `>=510 µm` = +0 |
| ML7 post-assessment planning | Keep device planning separate from ectasia eligibility; use the active Turkish ML7 reference and CERAI hinge amendment only after a favorable LASIK result | Not integrated | Status-independent module reports ring/pressure/blade and hinge recommendations without changing the ectasia disposition. For K spread `>4.00 D`, perpendicular-to-steep-axis hinge is primary; the `+10` temporal/nasal contingency requires projected RSB `>=300 µm` and PTA `<40%` and remains anatomy-dependent |
...
| Pachymetry policy | CERAI hard stop `<480 µm`; exactly 480 enters scoring | Implemented | Preserved and boundary-tested; all hard stops are accumulated even when other data are missing |
...
| Ablation estimate | CERAI conventions: Alcon EX500 `12 µm/D` at a 6.0-mm optical zone, `15 µm/D` at 6.5 mm, and `16.33 µm/D` at 7.0 mm; actual plan is preferred | Applied one estimate to every platform/zone | A zone-specific estimate runs only when EX500 and 6.0, 6.5, or 7.0 mm are explicitly documented; otherwise actual maximum ablation is required. The zone coefficients are binding CERAI operational conventions supplied for this implementation and are not independently validated by the supplied source set |
...
| PRK direct cohort | 310 µm is the cohort minimum, not a validated universal safe cutoff; PTA range to 35.28% is an evidence envelope | Not documented | CERAI 310 hard stop is explicitly labeled operational policy; `PRK PTA >35.28%` is labeled an evidence-gap flag, not proof of harm |
...
| Treatment-range rules | Apply binding CERAI intended-treatment sphere rules separately from the published scores | Rules were absent from the current engine | Intended sphere `<−10.00 D` and `>+6.00 D` are CERAI operational hard stops; exact −10.00/+6.00 boundaries are allowed by those rules. They are labeled CERAI policy, not published ERSS/PRK score weights |
...
10. CERAI evidence reviews and the master protocol supplied on 25 August 2026.
