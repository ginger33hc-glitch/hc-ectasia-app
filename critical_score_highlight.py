...
(function(){
  ...
  function markClinicalSignals(root){
    ...
    scope.querySelectorAll('tr').forEach(function(row){
      ...
      if(label==='bad-d final'){const result=cells[1],score=parseFloat((result.textContent||'').trim());result.classList.remove('hc-bad-normal','hc-bad-suspicious','hc-bad-abnormal');if(Number.isFinite(score)){if(score>=3.0){result.classList.add('hc-bad-abnormal');result.textContent=score.toFixed(2)+' — ABNORMAL';}else if(score>1.6){result.classList.add('hc-bad-suspicious');result.textContent=score.toFixed(2)+' — SUSPICIOUS';}else{result.classList.add('hc-bad-normal');result.textContent=score.toFixed(2)+' — NORMAL';}}}
      ...
      if(label==='morphology'||label==='anterior pattern'){const result=cells[1],t=(result.textContent||'').toUpperCase();result.classList.remove('hc-topography-normal','hc-topography-suspicious','hc-topography-abnormal');if(/ABNORMAL|ECTATIC|KERATOCONUS|PELLUCID|FFKC/.test(t))result.classList.add('hc-topography-abnormal');else if(/SUSPICIOUS|BORDERLINE|ASYMMETRIC|INFERIOR_STEEPENING|SRA|SRAX/.test(t))result.classList.add('hc-topography-suspicious');else if(/NORMAL|SYMMETRIC/.test(t))result.classList.add('hc-topography-normal');}
    });
