    ...
    with _EXPORT_LOCK:
        ...
        def patched_heading(document,text,level=1):
            ...
            if text==tr('Interpretation note'):
                orig_heading(document,tr('CERAI BAD-D reference points'),1)
                t=document.add_table(rows=1,cols=2);t.style='Table Grid';t.rows[0].cells[0].text=tr('Final BAD-D');t.rows[0].cells[1].text=tr('CERAI interpretation / action')
