"""CER-AI LASIK automatic planning fallback.

Plan A (default): flap 100 µm, optical zone 6.5 mm, transition zone 9.0 mm.
If Plan A fails OR LASIK PTA is >=40%, and there is no independent CER-AI hard-stop, re-evaluate with
...
def install(core: Any) -> None:
    ...
    def assess_eye_with_fallback(eye: Dict[str, Any], plan: Dict[str, Any], age: Any, patient_modifiers: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if len(sequence) > 1: warnings.append("Automatic LASIK fallback planning was activated because the preceding plan failed or reached the CER-AI PTA cutoff (>=40.0%). A fallback result does not erase the earlier configuration.")
