...
def possible_calendar_dates(value):
    ...
    # ISO / year-first numeric formats are unambiguous.
    match = re.fullmatch(r"(\d{4})[./-](\d{1,2})[./-](\d{1,2})", text)
    if match:
        parsed = _valid_iso(*match.groups())
        return {parsed} if parsed else set()

    # Day/month or month/day with a four-digit year. Dot-separated Pentacam
    # dates are treated as day.month.year; slash/hyphen forms remain ambiguous
    # when both interpretations are valid.
