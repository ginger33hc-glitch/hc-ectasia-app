...
| 32 | Refractive-group postoperative K policy | **PASS (LOCAL)** | Myopic estimated Kmean remains hard-stopped outside 36–48 D. Hyperopic Kmean is screening only: 48.00–48.99 D triggers verification and ≥49.00 D requires platform/surgeon confirmation, never an isolated hard stop. Mixed astigmatism completes with mandatory review rather than a missing-data loop. |
| 33 | One canonical refractive-group K calculation | **PASS (LOCAL)** | `clinical_core.safety.estimated_final_kmean_d` is the sole calculator: myopic 0.8× intended MRSE, hyperopic 1.0× intended MRSE screening, and no scalar result for mixed astigmatism. |
