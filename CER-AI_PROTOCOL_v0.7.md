...
- Report patient name uses one block: First Name / Last Name in the upper-left OD 4 Maps Refractive header (first supplied OD page). Use OS only when OD Four Maps is absent. An unreadable selected header requests surgeon entry; other-page name readings do not select or veto the report name. Patient ID is not requested, extracted, compared, or reported.
- Extract and compare patient name, printed age, date of birth, examination date/time, laterality, filename, and
  literal Pentacam QS.
...
PS3 procedure rule: thinnest pachymetry from 470 through 499 µm is Moderate; exactly 500 µm and above has no PS3 thickness escalation. Exactly one Moderate factor and no High factor ordinarily defers LASIK while PRK and SMILE remain allowed. The single exception is LASIK with thinnest pachymetry 490-499 µm inclusive as the sole PS3 Moderate factor: when ERSS, NICE, and Final BAD-D are each PASS or CAUTION, the LASIK result is PASS WITH CAUTION. Below 490 µm the exception does not apply. Two Moderate factors or one High factor defer LASIK, PRK, and SMILE. STOP-DEFER or incomplete ERSS, NICE, or Final BAD-D prevents the exception. Independent tissue-safety and eligibility gates remain controlling.
...
### ML7 keratometry source — 2026-09-10

ML7 reuses the eye's existing canonical `K1_D` and `K2_D`; it does not maintain or request duplicate ML7-specific K1/K2 fields. The canonical ML7 input selects their maximum as steepest K and minimum as flattest K. Kmax is never substituted. HWTW remains source-locked to the 4 Maps Refractive lower-left HWTW box and requires verified source or surgeon-confirmed provenance. If canonical K1, K2, or verified HWTW is genuinely missing, that original measurement is requested once; no ring is inferred when a required input is absent.
