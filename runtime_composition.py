...
COMPOSITION_PHASES={
 ...
 "reporting_and_readiness":("report_export_guard","critical_score_highlight","assessment_workflow","randleman_report_readiness_policy","ps3_report_policy"),
...
def compose(version:str):
 ...
 audit_log.install(core,archive_runtime);case_catalog.install(core,archive_runtime);historical_report.install(core,archive_runtime);research_export.install(core,archive_runtime);named_user_ui.install(core);pentacam_targeted_reread.install(core);erss_auto_read_policy.install(core)
