...
def _i_s_source(eye):
    ...
    if provenance or "I_S" in set(eye.get("table_verified_numeric_fields") or []): return "PENTACAM_LABELED_IS_INDEX"
    return None

def _prepared_eye(eye,plan):
    prepared=dict(eye); prepared["_erss_i_s_gate_required"]=(plan or {}).get("procedure")=="LASIK"
    manual=(plan or {}).get("surgeon_I_S_D"); prepared["_surgeon_I_S_invalid"]=manual is not None and not core.is_number(manual)
    if core.is_number(manual):
        prepared["I_S"]=float(manual); prepared["I_S_status"]="SURGEON_CONFIRMED"; prepared["I_S_source"]="SURGEON_ENTRY"
    return prepared
...
def _surgeon_confirmed_srax(eye):
    ...
    return value if any(str(item.get("source") or "").upper()=="SURGEON_CONFIRMED" for item in provenance if isinstance(item,dict)) else None
...
def _front_map_srax(eye):
    ...
    if core.is_number(value):
        d=float(value)
        if 0<=d<=90:
            return ("YES" if d>20 else "NO",d,"AXIAL_SAGITTAL_CURVATURE_FRONT",f"Front-map SRAX {d:.1f}°; criterion is strictly >20°.")
    ...
    if confirmed:return confirmed,None,"SURGEON_CONFIRMED_FRONT_MAP_REVIEW",f"Surgeon confirmed SRAX {'>20°' if confirmed=='YES' else 'is not >20°'} from the Axial/Sagittal Curvature (Front) map."
...
def scoring_morphology_with_i_s_evidence_gate(eye):
    ...
    evidence.append("Highest applicable single Randleman topography category selected from signed I-S and Front-map SRAX; categories are never added together.")
...
def required_tomography_missing_with_i_s(eye):
    missing=[item for item in _previous_required_tomography_missing(eye) if not any(token in str(item).lower() for token in ("morphology","topography category","asymmetric bow","inferior steep"))]
...
def _publish_validated_erss_topography(result,validated):
    """Handoff the validated shared topography category to the existing ERSS row.

    This does not create a second score. It replaces the previously unresolved
    topography row with the canonical point mapping and recomputes the same
    five-variable ERSS aggregate.
    """
    category=validated.get("category")
    if category=="UNCERTAIN":return
    points=core.lasik_topography_points(category)
    if points is None:return
    topo=result.setdefault("topography_classification",{})
    topo["scoring_category"]=category; topo["evidence"]=validated.get("evidence") or []
    erss=result.get("randleman_erss")
    if isinstance(erss,dict):
        rows=dict(erss.get("rows") or {}); rows["topography"]=points; erss["rows"]=rows
        missing=[name for name in _RANDLEMAN_ROWS if rows.get(name) is None]
        erss["missing_erss_inputs"]=missing
        total=None if missing else sum(int(rows[name]) for name in _RANDLEMAN_ROWS)
        erss["total"]=total; erss["category"]=core.score_category("LASIK",total) if total is not None else None
        erss["topography_category"]=category; erss["topography_evidence"]=validated.get("evidence") or []
        result["randleman_erss"]=erss
    score=result.get("score")
    if isinstance(score,dict):
        rows=dict(score.get("rows") or {}); rows["topography"]=points; score["rows"]=rows
        if all(core.is_number(rows.get(name)) for name in _RANDLEMAN_ROWS):
            total=sum(int(rows[name]) for name in _RANDLEMAN_ROWS); score["total"]=total; score["category"]=core.score_category("LASIK",total)
        result["score"]=score
...
def assess_eye_with_i_s_evidence(eye,plan,age,patient_modifiers):
    ...
    working=_prepared_eye(eye,plan or {})
    working["morphology"]="UNCERTAIN"; working["morphology_confidence"]="UNREADABLE"; working["morphology_evidence"]=[]; working["asymmetric_bow_tie"]="UNCERTAIN"; working["inferior_opposite_steepening_D"]=None
    result=_previous_assess_eye(working,plan,age,patient_modifiers)
    validated=scoring_morphology_with_i_s_evidence_gate(working)
    _publish_validated_erss_topography(result,validated)
    ist=_i_s_status(working); ss=validated.get("srax_status")
    ...
    result["erss_topography_evidence"]=rec
    result.setdefault("values",{}).update({"I_S_D":rec["I_S_D"],"I_S_status":rec["I_S_status"],"I_S_source":rec["I_S_source"],"SRAX_deg":rec["SRAX_deg"],"SRAX_status":rec["SRAX_status"],"SRAX_source":rec["SRAX_source"]})
    if working.get("_surgeon_I_S_invalid"):result.setdefault("missing",[]).append("valid numeric surgeon-confirmed I-S value")
    result["missing"]=[item for item in dict.fromkeys(result.get("missing") or []) if not any(token in str(item).lower() for token in ("morphology","topography category","asymmetric bow","inferior steep"))]
    return result
...
def install(runtime_core,prior_assess_eye=None):
    ...
    core=runtime_core; _previous_scoring_morphology=runtime_core.scoring_morphology; _previous_required_tomography_missing=runtime_core.required_tomography_missing; _previous_assess_eye=runtime_core.assess_eye; _prior_assess_eye=prior_assess_eye
    runtime_core.scoring_morphology=scoring_morphology_with_i_s_evidence_gate; runtime_core.required_tomography_missing=required_tomography_missing_with_i_s; runtime_core.assess_eye=assess_eye_with_i_s_evidence; runtime_core._erss_topography_evidence_policy_installed=True
